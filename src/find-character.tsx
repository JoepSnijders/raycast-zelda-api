...
const getMonsterData = (page: Number) => {
    const response = useFetch<Response>(`https://zelda.fanapis.com/api/characters?limit=1000&page=` + page);
...
export default function Command() {
    ...
    for (let i = 0; i < 32; i++) {
        ...
        if (pageData?.data) combinedData = [...combinedData, ...(pageData?.data as any[])];
    }
                    ...
                    <Detail.Metadata>
                        <Detail.Metadata.TagList title="Gender">
                            {props?.info?.gender === "Male" ? (
                                <Detail.Metadata.TagList.Item text={props?.info?.gender} color={"blue"} />
                            ) : (
                                <Detail.Metadata.TagList.Item text={props?.info?.gender} color={"pink"} />
                            )}
                        </Detail.Metadata.TagList>
                        <Detail.Metadata.TagList title="Race">
                            <Detail.Metadata.TagList.Item text={props?.info?.race} color={"purple"} />
                        </Detail.Metadata.TagList>
